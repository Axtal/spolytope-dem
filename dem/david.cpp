...
struct UserData
{
    ...
    double             T0;           ///< Initial time for the injection of water
    double             Tf;           ///< Final time for the injection of water
    double             eth;          ///< Strain threshold
...
void Setup (DEM::Domain & dom, void * UD)
{
    ...
    if (dom.Time > dat.T0 && dom.Time < 0.5*(dat.Tf+dat.T0))
    {
        for (size_t i=0; i<dom.BInteractons.Size(); i++)
        {
            if(dom.BInteractons[i]->valid)
            {
                 //dom.BInteractons[i]->eps = dat.eth*fabs(1-(dom.Time-dat.T0)/(0.5*(dat.Tf - dat.T0)));
                 dom.BInteractons[i]->eps = 0.0;
            }
        }
    }
}
