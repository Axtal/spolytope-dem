...
void Report (DEM::Domain & dom, void *UD)
{
    ...
    if (dom.idx_out==0)
    {
        ...
        dat.oss_ss << Util::_8s   << "e"    << Util::_8s << "Cn" << Util::_8s << "Nc" << Util::_8s << "Nsc" << Util::_8s << "Nr" << "\n";
    }
    if (!dom.Finished) 
    {
        // stress
        dat.oss_ss << Util::_8s << dom.Time << Util::_8s << dat.Sig(0) << Util::_8s << dat.Sig(1) << Util::_8s << dat.Sig(2);
        ...
        dat.oss_ss << Util::_8s << Cn << Util::_8s << Nc << Util::_8s << Nsc << Util::_8s << Nr;
...
int main(int argc, char **argv) try
{
    ...
    double Rminf;       // Fraction of the minimun radius to take polydisperse spheres
    ...
    double ratio;       // ratio between stress/time; if negative it will consider t0max and tc
    ...
    double alpf;        // Angle of the p q plane
    {
        ...
        infile >> Rminf;        infile.ignore(200,'\n');
        ...
        infile >> ratio;        infile.ignore(200,'\n');
    ...
    //setting the test times
    if (ratio>0.0)
    {
        T0max = ratio*p0max;
        Tc    = T0max + ratio*(p0max-pc);
        Tf    = Tc + Tf;
    }
