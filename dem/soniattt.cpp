...
void Report2 (DEM::Domain & dom, void *UD)
{
    ...
    FF <<  Util::_10_6 << "Fx" << Util::_8s << "Fy" << Util::_8s << "Fz" << Util::_8s << "P1" <<  Util::_8s << "P2" <<"\n";

    for (size_t i=0; i<dom.CInteractons.Size(); i++)
    {
        if (norm(dom.CInteractons[i]->Fnet)>0.0)
        {
            FF << dom.CInteractons[i]->Fnet(0) << Util::_8s << dom.CInteractons[i]->Fnet(1)  << Util::_8s <<  dom.CInteractons[i]->Fnet(2) 
...
int main(int argc, char **argv) try
{
    ...
    size_t nz;          // nz
    double rho;         // rho
    double p0max;       // Pressure for the isotropic compression
    double pc;          // Pressure for the isotropic compression
    double pfinal;      // Final pressure
    double phi;         // Enter friction angle
    double ocr;         // Enter ocr
    double T0max;       // Time span for the compression b
    double Tc;          // Time span for the compression b
    double Tp;          
    double Tode;
    ...
    double alpf;        // Angle of the p q plane
    {
        ...
        infile >> p0max;        infile.ignore(200,'\n');
        infile >> pc;           infile.ignore(200,'\n');
        infile >> pfinal;       infile.ignore(200,'\n');
        infile >> phi;          infile.ignore(200,'\n');
        infile >> ocr;          infile.ignore(200,'\n');
        infile >> T0max;        infile.ignore(200,'\n');
        infile >> Tc;           infile.ignore(200,'\n');
        infile >> Tp;           infile.ignore(200,'\n');
        infile >> Tode;         infile.ignore(200,'\n');
    ...
    // particle
    if      (ptype=="sphere")  dom.GenSpheres  (-1, Lx, nx, rho, "HCP", seed, fraction,0.1);
    ...
    //String fkey_as(filekey+"_as");
    //String fkey_bs(filekey+"_bs");
    ...
    double k0 = (1-sin(phi*M_PI/180.0))*pow(ocr,sin(phi*M_PI/180.0));
    double eta0 = 3.0*(1-k0)/(1+2*k0);
    double sv0 = p0max/ocr;
    double sh0 = k0*sv0;
    double po = (sv0 + 2*sh0)/3.0;
    double qo = sv0 - sh0;

     //stage 2: pcte path /////////////////////////////////////////////////////////////////////////
    String fkey_c(filekey+"_c");
    //String fkey_cs(filekey+"_cs");
    Vec3_t lf;
    pqth2L (pc, eta0*pc, thf*M_PI/180, lf, "cam");
    sigf   = lf(0), lf(1), lf(2);
    ResetEps  (dom,dat);
    SetTxTest (sigf, peps, depsdt, thf*M_PI/180, 90.0*M_PI/180, false, dat, dom);
    dat.tspan = Tp - dom.Time;
    dom.Solve     (/*tf*/Tp, /*dt*/dt, /*dtOut*/dtOut, &Setup, &Report, fkey_c.CStr(),RenderVideo);

     //stage 3: odeo path /////////////////////////////////////////////////////////////////////////
    String fkey_d(filekey+"_d");
    //String fkey_ds(filekey+"_ds");
    pqth2L (po, qo, thf*M_PI/180, lf, "cam");
    sigf   = lf(0), lf(1), lf(2);
    ResetEps  (dom,dat);
    SetTxTest (sigf, peps, depsdt, thf*M_PI/180, 90.0*M_PI/180, false, dat, dom);
    dat.tspan = Tode - dom.Time;
    dom.Solve     (/*tf*/Tode, /*dt*/dt, /*dtOut*/dtOut, &Setup, &Report, fkey_d.CStr(),RenderVideo);

    // stage 4: free path /////////////////////////////////////////////////////////////////////////
    String fkey_e(filekey+"_e");
    pqth2L (pfinal, (pfinal - po)*tan(alpf*M_PI/180)+qo, thf*M_PI/180.0, lf, "cam");
    sigf   = lf(0), lf(1), lf(2);
    ...
    dom.Solve     (/*tf*/Tf, /*dt*/dt, /*dtOut*/dtOut, &Setup, &Report2, fkey_e.CStr(),RenderVideo);
